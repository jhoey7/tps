...
class Reference extends CI_Controller {
	...
	function execute($type="", $act="", $id="", $met="") {
        if (!$this->session->userdata('LOGGED')) {
            ...
            if (strtolower($_SERVER['REQUEST_METHOD']) != "post") {
                ...
                $this->m_reference->execute($type, $act, $id, $met);
            }
        }
    }
	...
	public function kapal($act,$id){
		...
		if($act=="add"){
            ...
            $data['action'] = 'save';
            echo $this->load->view('content/reference/kapal/add', $data, true);
        }else if ($act == "update") {
			$data['title'] = 'UPDATE KAPAL';
            $data['id'] = $id;
			$data['action'] = 'update';
            $data['arrdata'] = $this->m_reference->execute('get', 'reff_kapal',$id);
            echo $this->load->view('content/reference/kapal/add', $data, true);
		}else{
			$this->load->model("m_reference");
			$arrdata = $this->m_reference->kapal($act, $id);
	...
	public function organisasi($act,$id){
		...
		$this->load->model('m_reference');
		if($act=="add"){
			$data['title'] = 'ENTRY CONSIGNEE';
            $data['action'] = 'save';
            echo $this->load->view('content/reference/organisasi/add', $data, true);
        }else if ($act == "update") {
			$data['title'] = 'UPDATE CONSIGNEE';
            $data['id'] = $id;
			$data['action'] = 'update';
            $data['arrdata'] = $this->m_reference->execute('get', 't_organisasi',$id);
            echo $this->load->view('content/reference/organisasi/add', $data, true);
		}else{
			$this->load->model("m_reference");
			$arrdata = $this->m_reference->organisasi($act, $id);
			$data = $this->load->view('content/newtable', $arrdata, true);
			if ($this->input->post("ajax") || $act == "post") {
				echo $arrdata;
			} else {
