...
class Dashboard extends CI_Controller {
	...
	public function index(){
		...
		$headers .= '<link rel="stylesheet" href="'.base_url().'assets/vendor/chartist-js/chartist.min.css?v2.1.0">';
        $headers .= '<link rel="stylesheet" href="'.base_url().'assets/vendor/chartist-plugin-tooltip/chartist-plugin-tooltip.min.css?v2.1.0">';
		$headers .= '<link rel="stylesheet" href="'.base_url().'assets/vendor/dashboard/v1.min.css">';
		#Plugins
		...
		$footers .= '<script src="'.base_url().'assets/vendor/chartist-js/chartist.min.js"></script>';
		$footers .= '<script src="'.base_url().'assets/js/components/tabs.min.js"></script>';
		if($this->session->userdata('LOGGED')){
			if($this->content==""){
				$this->load->model('m_execute');
				$data['disc_pack'] = $this->m_execute->get_data('coarri_codeco','t_cocostskms-gateinimp');
				$data['load_pack'] = $this->m_execute->get_data('coarri_codeco','t_cocostskms-gateoutimp');
				$data['in_pack'] = $this->m_execute->get_data('coarri_codeco','t_cocostskms-gateinexp');
				$data['out_pack'] = $this->m_execute->get_data('coarri_codeco','t_cocostskms-gateoutexp');
				$data['arr_hist'] = $this->m_execute->get_data('history');
