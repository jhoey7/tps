        ...
        <form name="form_data" id="form_data" class="form-horizontal" role="form" action="<?php echo site_url('execute/process/'.$action.'/kapal/'.$id); ?>" method="post" autocomplete="off" onsubmit="save_post('form_data','divtblkapal'); return false;" popup="1">
          <div class="panel-body container-fluid">
            <div class="row">
              ...
              <div class="form-group form-material">
                <label class="col-sm-3 control-label">VOYAGE IN</label>
                <div class="col-sm-9">
                  <input type="text" name="DATA[VOY_IN]" id="VOY_IN" mandatory="yes" class="form-control" placeholder="NO. VOYAGE / FLIGHT" value="<?php echo $arrdata['VOY_IN']; ?>">
          ...
          <input type="hidden" name="DATA[KD_DOK]" id="KD_DOK" readonly="readonly" value="1"/>
          <input type="hidden" name="action" id="action" readonly="readonly" value="<?php echo site_url('coarri/discharge/post'); ?>"/>
        </form>
      </div>
    </div>
  </div>
